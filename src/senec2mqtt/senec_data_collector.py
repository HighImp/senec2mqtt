    ...
    def _collect_data(self) -> dict:
        ...
        async def run(host):
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # only call "update" to avoid reading the system information to often
                # (see pysenec for details)
                await senec.update()
        ...
        if len(collected_data) != 1:
            raise RuntimeError(f"Expect exact one set of data from pysenec instance, "
                               f"but got: {len(collected_data)}!")
...
def main():
    ...
    ip_addr = input("Enter Device IP addr:")
    interval_s = int(input("Enter Period Time [s]:"))

    data_collector = SenecDataCollector(senec_ip=ip_addr, interval_s=interval_s)
    data_collector.start()
    time.sleep(5)
    data_collector.stop()
    data_collector.join(interval_s)

    print(data_collector.available_data())
    print(data_collector.get_all_data())
